...
class SignUpSuccess(TestCase):
    ...
    def test_sign_up_success(self):
        response = self.client.post('signup.html', {'userName': 'ben', 'password1': '11111', 'password2': '11111',
                                                    'group': 'Administrator'})
        self.assertEqual(response.url, 'homepage', msg='Successfully created an account.')


class UserExists(TestCase):
    def setUp(self):
        self.client = Client()
        self.user1 = Users.objects.create(userName='ben1', password1='11111', password2='11111', firstName='Ben', lastName='Franklin', phoneNum='1234567890', email='ben1@gmail.com', group='Administrator')

    def test_user_exists(self):
        response = self.client.post('signup.html', {'userName': 'ben1', 'password1': '11111', 'password2': '11111',
                                                    'group': 'Administrator'})
        self.assertEqual(response.url, 'login.html', response.context['message'],
                         msg="You have an account. Please log in.")
